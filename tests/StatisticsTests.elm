...
median =
    describe "median"
        [ fuzz3 float float float "median of 3 returns middle element" <|
            \fltA fltB fltC ->
                let
                    sortedLst =
                        List.sort [ fltA, fltB, fltC ]

                    middleElt =
                        List.tail sortedLst |> Maybe.andThen List.head |> Maybe.withDefault 0
                in
                expectAny
                    [ Statistics.median [ fltA, fltB, fltC ]
                        |> Maybe.withDefault 0
                        |> Expect.within tolerance middleElt
                    ]
        , fuzz2 float float "median of 2 returns avg of 2 elements" <|
            \fltA fltB ->
                let
                    medianRes =
                        fltA + fltB / 2
                in
                expectAny
                    [ Statistics.median [ fltA, fltB ]
                        |> Maybe.withDefault 0
                        |> Expect.within tolerance medianRes
                    ]
            ...
            \() ->
                ...
                expectAll
                    ...
                    , Statistics.median [ 1, 2, 3, 4 ]
                        |> orZero
                        |> Expect.within tolerance 2.5
                    , Statistics.median [ 1, 1 ]
                        |> orZero
                        |> Expect.within tolerance 1
                    , Statistics.median [ 1, 1, 1 ]
                        |> orZero
                        |> Expect.within tolerance 1
                    ]
        ]
